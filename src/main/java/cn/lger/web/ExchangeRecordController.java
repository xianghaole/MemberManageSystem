...
public class ExchangeRecordController {
    ...
    @GetMapping("/exchangeRecordWithFront")
    public String exchangeRecordWithFront(){
        return "exchangeRecordWithFront";
    }
    ...
    @PostMapping("/exchangeRecordWithFront")
    @ResponseBody
    public Page<ExchangeRecord> exchangeRecordWithFront(Integer currentPage, String memberId){
        if (currentPage == null || currentPage <0)
            currentPage = 0;
        return exchangeRecordService.findTransactionRecordByMemberId(currentPage, memberId);
    }
